...
async def run_etl(settings: Settings = None):
    ...
    for symbol in settings.SYMBOLS:
        ...
        if settings.USE_1M_CANDLES:
            ...
            has_label_data = len(df_1m) > 0
        else:
            df_1m = df.copy()  # Используем основной TF для лейблинга (намного быстрее)
            has_label_data = True

        if len(df) > 0 and len(htf_df) > 0 and has_label_data:
            if settings.USE_1M_CANDLES:
                logger.info(f"{symbol}: 1h={len(df)}, 4h={len(htf_df)}, 1m={len(df_1m)} candles")
            else:
                logger.info(f"{symbol}: 1h={len(df)}, 4h={len(htf_df)} candles (1m disabled)")
